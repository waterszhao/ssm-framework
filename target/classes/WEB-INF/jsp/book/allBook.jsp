        ...
        <div class="row clearfix">
            <div class="col-md-12 column">
                <table class="table table-hover table-striped">
                    ...
                    <tbody>
                    <c:forEach var="book" items="${books}" begin="${(currentPage-1)*maxLimit}" end="${currentPage*maxLimit}">
                    <tr>
                        <th>${book.bookID}</th>
                        <th>${book.bookName}</th>
            ...
            <div class="col-md-12 row">
                <c:if test="${totalPage <= 10}">
                    <c:forEach begin="0" end="${totalPage}" varStatus="status">
                        <a href="#?currentPage=${status.count}" class="btn-primary btn">${status.count}</a>
                    </c:forEach>
                </c:if>
            </div>
        </div>
